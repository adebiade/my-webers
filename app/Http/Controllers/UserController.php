...
class UserController extends Controller
{
  ...
  //Payment Controller
    ...
    return redirect()->route('user.arpProduct')->with('error', 'Payment failed or was cancelled');
...
/////////////////Old Payment codes
  ...
  $curl = curl_init();
     ...
     curl_setopt_array($curl, array(
      ...
      CURLOPT_HTTPHEADER => array(
        ...
        "Authorization: Bearer sk_test_062b0a7f826fe90a295dcd4fb2aff66640342620",
  ...
  if($err) {
    echo "cURL Error #:". $err;
     ...
     //echo $response;
      $result = json_decode($response);
          ...
          $payme = new paymentDetails();

       if ($result->data->status == 'success') {
          $payme->status = $result->data->status;
          $payme->reference = $result->data->reference;
          $lname = $result->data->customer->last_name;
          $fname = $result->data->customer->first_name;
          $payme->currency = $result->data->currency->USD;
          $payme->fullname = $lname.' '.$fname;
          $payme->cus_email = $result->data->customer->cus_email;
          $payme->basic =$result->basic;
          $payme->silver =$result->silver;
          $payme->premium = $result->premium;
          $payme->username = $result->username;
          //date_default_timezone_set('America/Los_Angeles')
          date_default_timezone_set('Africa/Lagos');
          $payme->date_time= date('m/d/Y h:i:s a', time());
          $payme.save();
        ...
        /*  $stmt = $con->prepare("INSERT INTO customer_details(status, reference, fullname, date_purchased, email) VALUES (?,?,?,?,?) ");
           ...
           $stmt->execute();*/
           if(!$payme.save()) {
             return redirect()->route('user.arpProduct')->with('status','Not successfull possibly there was an error');

            }
            else{
              return redirect()->route('user.arpProduct')->with('status','Successfully added! For proper check up your add will be posted with 24hours');
                exit;
              }
    }
  }
    ...
    {
      $aproducts = arProduct::orderBy('created_at','DESC')->paginate(12);
      //return view('inquiry',compact('arproducts'));

      return view('user.appartmentPage',compact('arproducts'));
    }
    public function arpProduct()
    {
        $arpproducts =arpProduct::where('username',Auth::user()->username)->paginate(10);
         //arpProduct::orderBy('created_at','DESC')->paginate(10);
        return view('user.arpproducts',compact('arpproducts'));
    }

    public function arproduct_add()
    {
      $categories = Category::select('id', 'name')->orderBy('name')->get();
      return view('user.arproduct-add',compact('categories'));
    }
      ...
      $product = new arpProduct();
       ...
       $product->Coworkingspaces = $request->Coworkingspaces;
        ...
        if($request->hasFile('images'))
        {
         ...
         $product->save();
         return redirect()->route('user.arpProduct')->with('status','Suuccessfully added! For proper check up your add will be posted with 24hours');
    ...
    public function arproduct_edit($id)
    {
      ...
      $categories = Category::select('id', 'name')->orderBy('name')->get();
        return view('user.arproduct-edit',compact('arproduct','categories'));
    ...
    public function arproduct_update(Request $request)
    {
      ...
      if($request->hasFile('images'))
      {
       ...
       $product->save();
       return redirect()->route('user.arpProduct')->with('status', 'Properties has been updated successfully!');
    }
    public function arproduct_delete($id)
    {
      ...
      $arproduct->delete();
      return redirect()->route('user.arpProduct')->with('status','Apartment has been deleted successfully');
    }
    ...
    {
        $bldproducts = bldProduct::where('username',Auth::user()->username)->paginate(10);

        //orderBy('created_at','DESC')->paginate(10);
        return view('user.bldproducts',compact('bldproducts'));
    }

    public function bldproduct_add()
    {
      $categories = Category::select('id', 'name')->orderBy('name')->get();
      return view('user.bldproduct-add',compact('categories'));
    }
          ...
          $bldproduct = new bldProduct();
           ...
           $bldproduct->widedoorwayshallways = $request->widedoorwayshallways;
            ...
            if($request->hasFile('images'))
            {
             ...
             $bldproduct->save();
             return redirect()->route('user.bldProduct')->with('status','Building has been added suuccessfully!');
    ...
    public function bldproduct_edit($id)
     {
       ...
       $categories = Category::select('id', 'name')->orderBy('name')->get();
         return view('user.bldproduct-edit',compact('ebldproduct','categories'));
     }
     public function bldproduct_update(Request $request)
     {
       ...
       if($request->hasFile('images'))
       {
        ...
        $edproduct->save();
        return redirect()->route('user.bldProduct')->with('status', 'Properties has been updated successfully!');
     }
     public function bldproduct_delete($id)
     {
       ...
       $bldproduct->delete();
       return redirect()->route('user.bldProduct')->with('status','Apartment has been deleted successfully');
     }
    ...
    {
      $categories = Category::select('id', 'name')->orderBy('name')->get();
      return view('user.vllproduct-add',compact('categories'));
    }

    public function vlProduct()
    {
        $vlproducts = vlProduct::where('username',Auth::user()->username)->paginate(10);

        //::orderBy('created_at','DESC')->paginate(10);
        return view('user.vlproducts',compact('vlproducts'));
    }
        ...
        $hmproduct->category_id = $request->category_id;
            ...
            $hmproduct->save();
             return redirect()->route('user.vlProduct')->with('status','Villa has been added suuccessfully!');
        ...
        public function GeneratevlProductThumbnailImage($image,$imageName)
       {
           ...
           public function vlproduct_edit($id)
          {
            ...
            $categories = Category::select('id', 'name')->orderBy('name')->get();
              return view('user.vlproduct-edit',compact('vlproduct','categories'));
    ...
    if($request->hasFile('images'))
    {
     ...
     $vlproduct->save();
     return redirect()->route('user.vlProduct')->with('status', 'Properties has been updated successfully!');
      }
          ...
          public function vlproduct_delete($id)
          {
            ...
            $vlproduct->delete();
            return redirect()->route('user.vlProduct')->with('status','Villa has been deleted successfully');
          }
        //Villa code end up
    ...
    {
      //$hmprdcts = Session::put('name', Auth::user()->username);
      //$session_id= Session::getId();
     //$category = User::select('username')->get();

        //$hmproducts = hmProduct::where('username',Auth::user()->username)->orderBy('created_at')->get();
        $hmproducts = hmProduct::where('username',Auth::user()->username)->paginate(10);

        //$hmproducts = hmProduct::orderBy('created_at','DESC')->paginate(10);
        return view('user.hmproducts',compact('hmproducts'));
    }
    public function hmproduct_add()
    {
      $categories = Category::select('id', 'name')->orderBy('name')->get();
      return view('user.hmproduct-add',compact('categories'));
    }
        ...
        public function hmproduct_store(Request $request)
        {
          $request->validate([
           ...
           $hmproduct->facadedesign = $request->facadedesign;
            ...
            $hmproduct->images = $gallery_images;
            $hmproduct->save();
             return redirect()->route('user.hmProduct')->with('status','Home has been added suuccessfully!');
       ...
       public function hmproduct_edit($id)
          {
            ...
            $categories = Category::select('id', 'name')->orderBy('name')->get();
              return view('user.hmproduct-edit',compact('hmproduct','categories'));
          ...
          public function hmproduct_update(Request $request)
          {
            ...
            if($request->hasFile('images'))
            {
             ...
             $hmproduct->save();
             return redirect()->route('user.hmProduct')->with('status', 'Properties has been updated successfully!');
          }
          public function hmproduct_delete($id)
          {
            ...
            $hmproduct->delete();
            return redirect()->route('user.hmProduct')->with('status','Home has been deleted successfully');
          }
    ...
    {
      $categories = Category::select('id', 'name')->orderBy('name')->get();
      return view('user.twnproduct-add',compact('categories'));
    }
    public function twnProduct()
    {
        $twnproducts = twnProduct::where('username',Auth::user()->username)->paginate(10);
        return view('user.twnproducts',compact('twnproducts'));
    }
    ...
    $hmproduct->privateentrance = $request->privateentrance;
        ...
        $hmproduct->images = $gallery_images;
        $hmproduct->save();
         return redirect()->route('user.twnProduct')->with('status','Villa has been added suuccessfully!');
    ...
    $categories = Category::select('id', 'name')->orderBy('name')->get();
      return view('user.twnproduct-edit',compact('twnproduct','categories'));
    }
        ...
        $hmproduct->save();
         return redirect()->route('user.twnProduct')->with('status','Villa has been added suuccessfully!');
          ...
          public function twnproduct_delete($id)
          {
            ...
            $twnproduct->delete();
            return redirect()->route('user.twnProduct')->with('status','Town house has been deleted successfully');
          }
    ...
    {
      $categories = Category::select('id', 'name')->orderBy('name')->get();
      return view('user.offproduct-add',compact('categories'));
    }
    public function offProduct()
    {
        $offproducts = offProduct::where('username',Auth::user()->username)->paginate(10);
        return view('user.offproducts',compact('offproducts'));
    }
    ...
    $hmproduct->restrooms = $request->restrooms;
        ...
        $hmproduct->images = $gallery_images;
        $hmproduct->save();
         return redirect()->route('user.offProduct')->with('status','An office has been added suuccessfully!');
   ...
   public function offproduct_edit($id)
   {
    ...
    $categories = Category::select('id', 'name')->orderBy('name')->get();
      return view('user.offproduct-edit',compact('offproduct','categories'));
    }
   ...
   $hmproduct->restrooms = $request->restrooms;
       ...
       $hmproduct->images = $gallery_images;
       $hmproduct->save();
        return redirect()->route('user.offProduct')->with('status','An office has been updated suuccessfully!');
   ...
   public function offproduct_delete($id)
   {
     ...
     $offproduct->delete();
     return redirect()->route('user.offProduct')->with('status','Office has been deleted successfully');
   }
   //Office code end up
    ...
    {
      $categories = Category::select('id', 'name')->orderBy('name')->get();
      return view('user.shproduct-add',compact('categories'));
    }
    public function shProduct()
    {
        $shproducts = shProduct::where('username',Auth::user()->username)->paginate(10);
        return view('user.shproducts',compact('shproducts'));
    }
                 ...
                 $shproduct->username = $request->username;
                 $shproduct->mobile = $request->mobile;
                 $shproduct->availability = $request->availability;
                 ...
                 $shproduct->strgestckroom = $request->strgestckroom;
                     ...
                     $shproduct->save();
                      return redirect()->route('user.shProduct')->with('status','Shop has been added suuccessfully!');
                ...
                public function shproduct_edit($id)
                {
                 ...
                 $categories = Category::select('id', 'name')->orderBy('name')->get();
                   return view('user.shproduct-edit',compact('shproduct','categories'));
                 }
                ...
                $shproduct->strgestckroom = $request->strgestckroom;
                    ...
                    $shproduct->images = $gallery_images;
                    $shproduct->save();
                     return redirect()->route('user.shProduct')->with('status',' Shop has been updated suuccessfully!');
                             ...
                             public function shproduct_delete($id)
                             {
                               ...
                               $shproduct->delete();
                               return redirect()->route('user.shProduct')->with('status','Shop has been deleted successfully');
                             }
                             //Shop code end up
    ...
    {
        $grgproducts = grProduct::where('username',Auth::user()->username)->paginate(10);
        //::orderBy('created_at','DESC')->paginate(10);
        return view('user.grproducts',compact('grgproducts'));
    }

    public function grgproduct_add()
    {
      $categories = Category::select('id', 'name')->orderBy('name')->get();
      return view('user.grgproduct-add',compact('categories'));
    }
    ...
    $grgproduct->username = $request->username;
    $grgproduct->mobile = $request->mobile;
    $grgproduct->availability = $request->availability;
    ...
    $grgproduct->garagedoor = $request->garagedoor;
        ...
        $grgproduct->save();
         return redirect()->route('user.grProduct')->with('status','A Garage has been added suuccessfully!');
   ...
   public function grproduct_edit($id)
   {
    ...
    $categories = Category::select('id', 'name')->orderBy('name')->get();
      return view('user.grgproduct-edit',compact('grgproduct','categories'));
    }
 ...
 $grgproduct->delete();
 return redirect()->route('user.grProduct')->with('status','Garage has been deleted successfully');
 }
 ...
 public function agentforms()
 {
    ...
    return view('user.agentform',compact('agentlists'));
 }
 public function agentform_add()
 {
   $categories = Category::select('id', 'name')->orderBy('name')->get();
   return view('user.agentform-add',compact('categories'));
 }

 public function agentform_store(Request $request)
  {
    $request->validate([
     'name' => 'required',
     'mobile'=>'required',
     ...
     $current_timestamp = Carbon::now()->timestamp;
      ...
      $agmform->save();
       return redirect()->route('user.agentforms')->with('status','Suuccessfully added! For proper check up your add will be posted with 24hours');
  ...
  public function agentform_edit($id)
  {
    ...
    //$categories = Category::select('id', 'name')->orderBy('name')->get();
      return view('user.agentform-edit',compact('agentform'));
   ...
   public function agentform_delete($id)
   {
     ...
     $agentimage->delete();
     return redirect()->route('user.agentform')->with('status','your profile has been deleted successfully');
   }

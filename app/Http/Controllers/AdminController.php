    ...
    public function index()
    {
      ...
      $dashBoardDatas = DB::select("Select sum(total) As TotalAmount,
        ...
        $monthlyDatas = DB::select("SELECT M.id As MonthNo, M.name As MonthName,
                            ...
                            LEFT JOIN (Select DATE_FORMAT(created_at, 'Ib') As MonthName,
                            MONTH(created_at) As MonthNo,
                            sum(total) As TotalAmount,
                            sum(if(status='ordered',total,0)) As TotalOrderedAmount,
                            sum(if(status='delivered',total,0)) As TotalDeliveredAmount,
                            sum(if(status='canceled',total,0)) As TotalCanceledAmount
                            From Orders WHERE YEAR(created_at)=YEAR(NOW()) GROUP BY YEAR(created_at), MONTH(created_at),DATE_FORMAT(created_at, 'Ib')
       ...
       return view('admin.index',compact('orders','dashBoardDatas','AmountM','OrderedAmount','DeliveredAmount','CanceledAmount','TotalAmount','TotalOrderedAmount','TotalDeliveredAmount','TotalCanceledAmount'));
    }
    ...
    public function brand_update(Request $request)
    {
     ...
     public function product_store(Request $request)
     {
      $request->validate([
         ...
         $product->save();
         return redirect()->route('admin.products')->with('status','product has been addedd sucessfully');
     }
